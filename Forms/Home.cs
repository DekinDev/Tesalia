...
namespace Tesalia_Redes_App
{
    public partial class Form1 : Form
    {
        ...
        void SaveJornada()
        {
            ...
            if (string.IsNullOrEmpty(EM0.Text) == false)
            {
                MiniJornada = 1;
            }
            if (string.IsNullOrEmpty(EM0.Text) == false)
        ...
        private void EliminarJornada_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(JornadaFiles.Text) == false)
            {
                DialogResult dialogResult = MessageBox.Show("¿Quieres eliminar este archivo de jornada?", "Tesalia Redes", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (dialogResult == DialogResult.Yes)
                {
                    try
                    {
                        File.Delete(Application.StartupPath + @"\Jornadas\" + JornadaFiles.Text);
