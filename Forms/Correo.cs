...
namespace Tesalia_Redes_App
{
    public partial class Form3 : Form
    {
        ...
        {
            if (string.IsNullOrEmpty(Properties.Settings.Default.MailMail) == false)
            {
                if (string.IsNullOrEmpty(Properties.Settings.Default.MailPass) == false)
                {
                    if (string.IsNullOrEmpty(Asunto.Text) == false)
                    {
                        if (string.IsNullOrEmpty(Mensaje.Text) == false)
                        {
                            ...
                            SmtpClient smtpclient = new SmtpClient("217.116.0.228")
                            {
                                ...
                                Credentials = new System.Net.NetworkCredential(Properties.Settings.Default.MailMail, Properties.Settings.Default.MailPass)
                            };
